...
我们对llava-v1.5-7b和llava-v1.5-13b两个模型使用了lora算法进行微调, 我们使用的数据集是bdd1k\_by\_Qiancai, 我们使用了llava官方提供的微调脚本, 并修改了一些训练参数, 对上述两个模型各自进行了10个epoch的微调, 微调过程中的损失如下:
\begin{figure}[H]
  \centering
  \begin{minipage}[b]{0.49\textwidth}
    \includegraphics[width=\textwidth]{fig/7b.png}
    \caption{7b规模模型训练过程中的loss}
    \label{fig:7b规模模型训练过程中的loss}
  \end{minipage}
  \hfill
  \begin{minipage}[b]{0.49\textwidth}
    \includegraphics[width=\textwidth]{fig/13b.png}
...
为了方便模型的调用与测试, 我们使用python的curses库写了一个朴素的gui界面进行模型的调用

...
\begin{enumerate}
  ...
  \item \textbf{数据划分：}
        \begin{itemize}
          \item 数据集被划分为训练集、验证集、测试集和小型集，具体信息如下：
                \begin{itemize}
  ...
  \item \textbf{数据组织：}
        \begin{itemize}
          \item 解压后，数据组织结构如下：
                \begin{verbatim}
    ├── val                    -- CODA2022 val (只使用图像)
    │   ├── images
    │   │   ├── *.jpg
    ├── test                   -- CODA2022 test (只使用图像)
    │   ├── images
    │   │   ├── *.jpg
    ├── CODA-LM
    │   ├── Train              -- CODA-LM 训练集 (使用 CODA2022 val 的 4884 图像)
    │   │   ├── val_*.json
    │   ├── Val                -- CODA-LM 验证集 (使用 CODA2022 test 的 4384 图像)
    │   │   ├── test_*.json
    │   ├── Test               -- CODA-LM 测试集 (使用 CODA2022 test 的 500 图像)
    │   │   ├── test_*.json
    │   ├── Mini               -- CODA-LM 小型集 (验证集的 50 图像子集)
    │   │   ├── test_*.json
    \end{verbatim}
        \end{itemize}
